...
class Filler:
    ...
    def fill_student(self, offset, gen=True, rows=100_000):
        ...
        for i in range(rows):
            ...
            data = (
                ...
                last,
                gender,
                self.department_list[i % dlen].strip(),
                date_delta.strftime("%Y-%m-%d"),
                credit,
            )
            ...
            data3 = (
                count,
                (first.lower() + last.lower() + addr["postalCode"] + "@sqlu.edu"),
                (
                    "("
    ...
    def teaches(self, offset):
        self.cursor.execute(
            "SELECT dept_name FROM department LIMIT 1 OFFSET " + str(offset) + ";"
        )
        for dept in self.cursor.fetchall():
            self.cursor.execute(
                'SELECT ID FROM instructor WHERE dept_name = "' + dept[0] + '";'
            )
            teachers = self.cursor.fetchall()
            teach_nums = len(teachers)
            teach_select = 0
            self.cursor.execute(
                'SELECT course_id FROM course WHERE dept_name = "' + dept[0] + '";'
            )
            count = 0
            for course in self.cursor.fetchall():
                ...
                for class_ in self.cursor.fetchall():
                    ...
                    data = (
                        str(teachers[teach_select % teach_nums][0]),
                        str(course[0]),
                        str(class_[0]),
                    )
                    self.insert(sql, data)
    ...
    def takes(self, offset):
        ...
        for dept in self.cursor.fetchall():
            self.cursor.execute(
			 ...
			 WHERE course.dept_name LIKE "'
                + dept[0]
                + '";'
            )
            classes = self.cursor.fetchall()
            self.cursor.execute(
                'SELECT ID FROM student WHERE dept_name = "' + dept[0] + '";'
            )
